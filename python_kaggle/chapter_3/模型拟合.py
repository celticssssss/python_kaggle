...
'''
由于猜测，可能披萨的米娜与与售价的线性关系更加明显，所以试图将原特征升高一个维度
'''
# 使用2次多项式线性回归模型在披萨训练样本上进行拟合
...
'''
因为2次型表现更好，所以考虑进一步提升特征维度，看看效果怎么样
'''
...
print('the socer is %f',regressor.score(X_test, y_test))

# 使用测试数据对2次多项式回归模型的性能进行评估。
X_test_poly2 = poly2.transform(X_test)
print('the socer is %f',regressor_poly2.score(X_test_poly2, y_test))

# 使用测试数据对4次多项式回归模型的性能进行评估。
X_test_poly4 = poly4.transform(X_test)
print('the socer is %f',regressor_poly4.score(X_test_poly4, y_test))

'''
使用正则化进行处理
'''
# 从sklearn.linear_model中导入Lasso。L1正则化
from sklearn.linear_model import Lasso

# 从使用默认配置初始化Lasso。
lasso_poly4 = Lasso(alpha=1, max_iter=100)
...
# 从sklearn.linear_model导入Ridge。L2正则化
